...
btScalar yaw=0.0f,pitch=0.f,roll=0.f;
...
//#ifndef TEST_NOT_ADDING_OBJECTS_TO_WORLD
	...
	for (i=0;i<numManifolds;i++)
	{
		...
		for (int j=0;j<numContacts;j++)
		{
            ...
            btScalar g = pt.m_appliedImpulse;
            if (g > 0.0f)
                printf("IMP=%f\n",g);
...
{
	objects[0].getWorldTransform().setOrigin(btVector3(1.6f,3.f,0.f));
	
	btQuaternion rotA(1.739f,-0.204f,0.587f,0.257f);
	rotA.normalize();
